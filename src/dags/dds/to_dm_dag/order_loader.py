        ...
        with self._db.client().cursor(row_factory=class_row(OrderObj)) as cur:
            cur.execute(
                """
                    ...
                    (select DISTINCT ON (user_id) id from dds.dm_users du where du.user_id = t.user_id ORDER BY user_id, id) user_id,
                    (select DISTINCT ON (restaurant_id) id from dds.dm_restaurants dr where dr.restaurant_id = t.restaurant_id ORDER BY restaurant_id, id) restaurant_id,
                    (select DISTINCT ON (ts) id from dds.dm_timestamps dt where dt.ts = t.order_ts ORDER BY ts, id) timestamp_id
                    from (
                    select 
                    id AS id,
...
class OrdersLoader:
    ...
    def load_data(self):
        # Открываем транзакцию: если код в блоке with пройдет успешно => будет закоммичена
        # Если возникнет ошибка, произойдет откат изменений (rollback транзакции).
        with self.pg_dest.connection() as conn:

            # Получаем состояние загрузки
            # Если настройки еще нет, создаем ее.
